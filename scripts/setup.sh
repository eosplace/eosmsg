...
echo "Creating ctgbarbeting user and registring contract" 
...
cleos -u http://${NODE_EOS}:8800 --wallet-url http://${NODE_EOS}:${PORT} set contract ctgbarbeting ../contract/ -p ctgbarbeting
...
cleos -u http://${NODE_EOS}:8800 --wallet-url http://${NODE_EOS}:${PORT} wallet import --private-key 5JnGRRJmehWevC32gNSWBDQEhH97Y2C9GHXPAPp12iqm5W4KziF
...
echo "Allow ctgbarbeting contract to make transfers on players behalf - the mobile app must be this permission setup for all accounts" 
cleos -u http://${NODE_EOS}:8800 --wallet-url http://${NODE_EOS}:${PORT} set account permission player1 active '{"threshold":1, "keys":[{"key":"EOS6pXfwZ976YHeNiRgWewX3ZTmmyTD5ej8RkWoQDQXw4J63zNnov", "weight":1}], "accounts": [{"permission":{"actor":"ctgbarbeting","permission":"eosio.code"},"weight":1}]}' owner -p player1
cleos -u http://${NODE_EOS}:8800 --wallet-url http://${NODE_EOS}:${PORT} set account permission player2 active '{"threshold":1, "keys":[{"key":"EOS6AcefxUgbQQeK7hXRTyAn2gxgZmsRyKZLVFeUnWrY1p4oMo2Ee", "weight":1}], "accounts": [{"permission":{"actor":"ctgbarbeting","permission":"eosio.code"},"weight":1}]}' owner -p player2
cleos -v -u http://${NODE_EOS}:8800 --wallet-url http://${NODE_EOS}:${PORT} set account permission player3 active '{"threshold":1, "keys":[{"key":"EOS6ZXD8W9ttK3PzypRpGmE7Y2pQq151k9jjEQc4LQq1rivk2Fpu1", "weight":1}], "accounts": [{"permission":{"actor":"ctgbarbeting","permission":"eosio.code"},"weight":1}]}' owner -p player3
